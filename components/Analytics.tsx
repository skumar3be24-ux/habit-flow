...
export default function Analytics({ habits }: AnalyticsProps) {
  const stats = useMemo(() => {
    ...
    // Setup last 14 days for AreaChart to guarantee exact 14-day X-axis
    ...
    habits.forEach(h => {
      ...
      dates.forEach(dateStr => {
        // Safely parse date to avoid timezone shift
    ...
    // Productivity Score: (Total Completions / (Total Habits * 30 Days)) * 100
    const rawProdScore = habits.length > 0 ? Math.round((totalCompletions / (habits.length * 30)) * 100) : 0;
    const productivityScore = Math.min(100, rawProdScore); // Cap at 100%

    // Sort Habit Breakdown by highest count
  ...
  if (!habits || habits.length === 0) {
    return (
      <div className="py-20 text-center border border-white/5 rounded-[3rem] bg-white/5">
        <h3 className="text-xl font-bold text-white mb-2">No data yet</h3>
        <p className="text-slate-500 uppercase text-[10px] tracking-widest font-bold">Add habits to unlock your analytics dashboard</p>
      </div>
    );
  }
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        <div className="bg-white/5 border border-white/10 p-6 md:p-8 rounded-[2rem] shadow-xl">
          <p className="text-[10px] font-black text-slate-500 uppercase mb-2 tracking-[0.2em]">Productivity Score</p>
          <div className="text-3xl md:text-4xl font-black text-indigo-400">{stats.productivityScore}%</div>
        </div>
        <div className="bg-white/5 border border-white/10 p-6 md:p-8 rounded-[2rem] shadow-xl">
          <p className="text-[10px] font-black text-slate-500 uppercase mb-2 tracking-[0.2em]">Total Completions</p>
          <div className="text-3xl md:text-4xl font-black text-white">{stats.totalCompletions}</div>
        </div>
        <div className="bg-white/5 border border-white/10 p-6 md:p-8 rounded-[2rem] shadow-xl">
          <p className="text-[10px] font-black text-slate-500 uppercase mb-2 tracking-[0.2em]">Active Habits</p>
          <div className="text-3xl md:text-4xl font-black text-orange-400">{stats.activeHabits}</div>
        </div>
        <div className="bg-white/5 border border-white/10 p-6 md:p-8 rounded-[2rem] shadow-xl">
          <p className="text-[10px] font-black text-slate-500 uppercase mb-2 tracking-[0.2em]">Best Global Streak</p>
          <div className="text-3xl md:text-4xl font-black text-green-400">{stats.globalBestStreak} 🔥</div>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        
        {/* 2. DAILY DISCIPLINE (AREA CHART) */}
      ...
      {/* 4. HABIT BREAKDOWN (HORIZONTAL BARS) */}
      <div className="bg-white/5 border border-white/10 p-6 md:p-8 rounded-[3rem] shadow-xl">
