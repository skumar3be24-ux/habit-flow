...
import { format, startOfMonth, endOfMonth, eachDayOfInterval, isFuture, isToday, getDay, addMonths, subMonths, isSameDay } from 'date-fns';
      ...
      <div className="flex justify-between items-start mb-8">
        ...
        <button 
          ...
          className={`p-3 rounded-2xl transition-all duration-300 flex items-center gap-2 ${
            isConfirming ? 'bg-red-500 text-white px-5 scale-105 shadow-lg shadow-red-500/20' : 'bg-white/5 text-slate-500 hover:text-red-400 hover:bg-red-400/10'
          }`}
        >
          {isConfirming ? <><span className="text-[10px] font-black uppercase tracking-tighter">Confirm?</span><X size={16} strokeWidth={3} /></> : <Trash2 size={20} />}
        </button>
      </div>
      ...
      <div className="max-w-[400px]">
        <div className="flex justify-between items-center mb-3 px-1">
          ...
          <div className="flex gap-1">
            ...
            <button onClick={() => setViewDate(addMonths(viewDate, 1))} className="p-1.5 hover:bg-white/10 rounded-lg text-slate-400 transition-colors" disabled={isSameDay(startOfMonth(viewDate), startOfMonth(new Date()))}><ChevronRight size={16}/></button>
          </div>
        </div>

        <div className="grid grid-cols-7 gap-1.5 text-center mb-2">
          {['S','M','T','W','T','F','S'].map((d, i) => <span key={i} className="text-[10px] font-black text-slate-500">{d}</span>)}
        </div>
          ...
          {daysInMonth.map((date) => {
            ...
            return (
              <button
                ...
                className={`aspect-square rounded-lg text-[11px] font-black transition-all flex items-center justify-center relative
